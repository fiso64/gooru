...
On non-Windows systems, owner-only `0400`/`0600` permissions are the simplest recommendation. Gooru also accepts group-readable `0440`/`0640` files for deployments such as a root-owned secret exposed to a dedicated, trusted Gooru service group. Group write/execute permissions and all permissions for other users are rejected: a world-readable recovery key would defeat protected mode's local-user threat model. Secret-manager paths work well with declarative deployments; for example, NixOS/agenix can assign `services.gooru.settings.encryption.key_file` to an age-secret path without putting the key itself in the Nix store. Configure such mounts to expose the secret only to the Gooru user or trusted service group.
