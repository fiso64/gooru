...
// UntagFilesByQuery removes tags from files matching an expression.
...
func (c *Client) RenameTag(oldName, newName string) error {
	...
	return c.store.RenameTag(query.ParseTag(oldName), query.ParseTag(newName))
}

// SetTagsForFiles sets tags for multiple files, replacing any existing ones.
...
type fileStateAnalysis struct {
	...
	potentialMoves    map[string]string
}
	...
	for _, absPath := range absPaths {
		...
		if wasHashed {
			if result.Err != nil {
				if !processedPaths[originalPath] {
					if progressCb != nil {
						progressCb(originalPath, fmt.Errorf("hashing failed: %v", result.Err))
					}
					processedPaths[originalPath] = true
				}
				continue
...
func (c *Client) executeTaggingTransaction(analysis *fileStateAnalysis, tags []string, kind opKind, tasks []BackgroundTaskRequest, stateBuilder BackgroundOperationTransactionStateBuilder, finalizer taggingTransactionFinalizer) (int64, map[string]string, error) {
	movesHandled := make(map[string]string)
	tx, err := c.store.Begin()
	if err != nil {
		return 0, nil, err
	}
