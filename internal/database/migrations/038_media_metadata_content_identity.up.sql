-- Persist media properties once per content identity. Locations remain the
-- presentation context, but duplicate paths for identical bytes share one row.
...
CREATE TABLE media_metadata_by_content (
...
INSERT INTO media_metadata_by_content (
...
SELECT l.content_hash, mm.media_kind, mm.mime_type, mm.image_width, mm.image_height,
...
WHERE NOT EXISTS (
    SELECT 1
    FROM media_metadata newer
    JOIN locations newer_location ON newer_location.id = newer.location_id
    WHERE newer_location.content_hash = l.content_hash
      AND (
          newer.updated_at > mm.updated_at OR
          (newer.updated_at = mm.updated_at AND newer.location_id > mm.location_id)
      )
);

DROP TABLE media_metadata;
ALTER TABLE media_metadata_by_content RENAME TO media_metadata;
CREATE INDEX idx_media_metadata_kind_lower_content
    ON media_metadata(lower(media_kind), content_hash);
...
SELECT CASE
        ...
        ELSE coalesce(
            (SELECT media_kind FROM media_metadata WHERE content_hash = l.content_hash),
            CASE
...
BEGIN
    ...
    VALUES (CASE
        ...
        ELSE coalesce(
            (SELECT media_kind FROM media_metadata WHERE content_hash = NEW.content_hash),
            CASE
...
BEGIN
    UPDATE kind_counts SET files_count = files_count - 1
    WHERE kind = CASE
        WHEN lower(OLD.extension) = '.cbz' THEN 'comic'
        ELSE coalesce(
            (SELECT media_kind FROM media_metadata WHERE content_hash = OLD.content_hash),
            CASE
    ...
    END;
    DELETE FROM kind_counts WHERE files_count <= 0;
END;

CREATE TRIGGER kind_counts_location_identity_update
AFTER UPDATE OF content_hash, extension ON locations
WHEN OLD.content_hash IS NOT NEW.content_hash OR OLD.extension IS NOT NEW.extension
BEGIN
    UPDATE kind_counts SET files_count = files_count - 1
    WHERE kind = CASE
        WHEN lower(OLD.extension) = '.cbz' THEN 'comic'
...
BEGIN
    ...
    SET files_count = files_count - (
        ...
        WHERE l.content_hash = NEW.content_hash
          AND lower(l.extension) <> '.cbz'
          AND CASE
        WHEN lower(l.extension) = '.gif' THEN 'gif'
        WHEN lower(l.extension) IN ('.jpg', '.jpeg', '.png', '.webp', '.bmp', '.tif', '.tiff', '.heic', '.heif') THEN 'photo'
        WHEN lower(l.extension) IN ('.mp4', '.mov', '.avi', '.mkv', '.webm', '.flv', '.wmv', '.mpeg', '.mpg') THEN 'video'
        ELSE 'other'
    END = kind_counts.kind
    )
    WHERE EXISTS (
    ...
    INSERT INTO kind_counts (kind, files_count)
    SELECT NEW.media_kind, COUNT(*) FROM locations l
    WHERE l.content_hash = NEW.content_hash AND lower(l.extension) <> '.cbz'
    HAVING COUNT(*) > 0
    ON CONFLICT(kind) DO UPDATE SET files_count = files_count + excluded.files_count;
    DELETE FROM kind_counts WHERE files_count <= 0;
END;
...
BEGIN
    UPDATE kind_counts SET files_count = files_count - (
        SELECT COUNT(*) FROM locations l
        WHERE l.content_hash = NEW.content_hash AND lower(l.extension) <> '.cbz'
    ) WHERE kind = OLD.media_kind;
    INSERT INTO kind_counts (kind, files_count)
    SELECT NEW.media_kind, COUNT(*) FROM locations l
    WHERE l.content_hash = NEW.content_hash AND lower(l.extension) <> '.cbz'
    HAVING COUNT(*) > 0
...
BEGIN
    UPDATE kind_counts SET files_count = files_count - (
        SELECT COUNT(*) FROM locations l
        WHERE l.content_hash = OLD.content_hash AND lower(l.extension) <> '.cbz'
    ) WHERE kind = OLD.media_kind;
    INSERT INTO kind_counts (kind, files_count)
    SELECT CASE
        WHEN lower(l.extension) = '.gif' THEN 'gif'
...
BEGIN
    ...
    END, COUNT(*)
    FROM locations l WHERE l.content_hash = NEW.content_hash
    GROUP BY 2
...
BEGIN
    ...
    SET files_count = files_count - (
        SELECT COUNT(*) FROM locations l
        WHERE l.content_hash = OLD.content_hash AND CASE
        WHEN lower(l.extension) = '.cbz' THEN 'comic'
        ELSE coalesce(
            (SELECT media_kind FROM media_metadata WHERE content_hash = l.content_hash),
            CASE
...
BEGIN
    ...
    END, 1
    FROM content_tags ct WHERE ct.content_hash = NEW.content_hash
...
BEFORE DELETE ON locations
BEGIN
    UPDATE tag_kind_counts SET files_count = files_count - 1
...
CREATE TRIGGER tag_kind_counts_location_identity_update
AFTER UPDATE OF content_hash, extension ON locations
WHEN OLD.content_hash IS NOT NEW.content_hash OR OLD.extension IS NOT NEW.extension
BEGIN
    UPDATE tag_kind_counts SET files_count = files_count - 1
...
BEGIN
    ...
    INSERT INTO tag_kind_counts (tag_id, kind, files_count)
    SELECT ct.tag_id, NEW.media_kind, COUNT(l.id)
    FROM content_tags ct JOIN locations l ON l.content_hash = ct.content_hash
    WHERE ct.content_hash = NEW.content_hash AND lower(l.extension) <> '.cbz'
    GROUP BY ct.tag_id HAVING COUNT(l.id) > 0
    ON CONFLICT(tag_id, kind) DO UPDATE SET files_count = files_count + excluded.files_count;
    DELETE FROM tag_kind_counts WHERE files_count <= 0;
END;
...
BEGIN
    ...
    END, COUNT(l.id)
    FROM content_tags ct JOIN locations l ON l.content_hash = ct.content_hash
    WHERE ct.content_hash = OLD.content_hash AND lower(l.extension) <> '.cbz'
    GROUP BY ct.tag_id, 2
    ON CONFLICT(tag_id, kind) DO UPDATE SET files_count = files_count + excluded.files_count;
    DELETE FROM tag_kind_counts WHERE files_count <= 0;
END;
...
BEGIN
    INSERT INTO tag_key_kind_counts (key, kind, files_count)
    SELECT (SELECT key FROM tags WHERE id = NEW.tag_id), CASE
        WHEN lower(l.extension) = '.cbz' THEN 'comic'
        ELSE coalesce(
            (SELECT media_kind FROM media_metadata WHERE content_hash = l.content_hash),
            CASE
...
CREATE TRIGGER tag_key_kind_counts_location_identity_update
AFTER UPDATE OF content_hash, extension ON locations
WHEN OLD.content_hash IS NOT NEW.content_hash OR OLD.extension IS NOT NEW.extension
BEGIN
    UPDATE tag_key_kind_counts SET files_count = files_count - 1
...
BEGIN
    ...
    INSERT INTO tag_key_kind_counts (key, kind, files_count)
    SELECT tagged.key, NEW.media_kind, COUNT(l.id)
    FROM (
        SELECT DISTINCT t.key FROM content_tags ct JOIN tags t ON t.id = ct.tag_id
        WHERE ct.content_hash = NEW.content_hash
    ) tagged
    JOIN locations l ON l.content_hash = NEW.content_hash
    WHERE lower(l.extension) <> '.cbz'
    GROUP BY tagged.key HAVING COUNT(l.id) > 0
    ON CONFLICT(key, kind) DO UPDATE SET files_count = files_count + excluded.files_count;
    DELETE FROM tag_key_kind_counts WHERE files_count <= 0;
END;
...
BEGIN
    ...
    JOIN locations l ON l.content_hash = OLD.content_hash
    WHERE lower(l.extension) <> '.cbz'
    GROUP BY tagged.key, 2
    ON CONFLICT(key, kind) DO UPDATE SET files_count = files_count + excluded.files_count;
    DELETE FROM tag_key_kind_counts WHERE files_count <= 0;
...
BEGIN
    ...
    SELECT t.key, COUNT(DISTINCT ct.content_hash)
    FROM tags t JOIN content_tags ct ON ct.tag_id = t.id
    WHERE t.value != '' AND (t.key = OLD.key COLLATE NOCASE OR t.key = NEW.key COLLATE NOCASE)
    GROUP BY t.key;
    DELETE FROM tag_key_kind_counts
    WHERE key = OLD.key COLLATE NOCASE OR key = NEW.key COLLATE NOCASE;
    INSERT INTO tag_key_kind_counts (key, kind, files_count)
