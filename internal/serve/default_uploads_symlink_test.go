...
func TestDefaultUploadDirectoryRejectsSymlinkedParent(t *testing.T) {
	...
	if err := os.Mkdir(realRoot, 0750); err != nil {
		t.Fatal(err)
	}
	...
	if err == nil {
		t.Fatal("must reject a symlinked parent")
	}
...
func TestDefaultUploadDirectoryRejectsSymlinkedTarget(t *testing.T) {
	...
	if err := os.Mkdir(realTarget, 0755); err != nil {
		t.Fatal(err)
	}
	path := filepath.Join(root, "uploads")
	if err := os.Symlink(realTarget, path); err != nil {
		t.Fatal(err)
	}
	if err := prepareDefaultUploadDir(path); err == nil {
		t.Fatal("must reject a symlinked upload target")
	}
	...
	if info.Mode().Perm() != 0755 {
		t.Fatalf("symlink destination permissions changed: %v", info.Mode())
	}
}
