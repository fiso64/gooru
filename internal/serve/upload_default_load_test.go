		...
		t.Run(tc.name, func(t *testing.T) {
			...
			if tc.errPart != "" {
				if err == nil || !strings.Contains(err.Error(), tc.errPart) {
					t.Fatalf("expected %q error, got %v", tc.errPart, err)
				}
				return
			}
			if err != nil {
			...
			if cfg.Uploads.Enabled != tc.enabled || len(cfg.Uploads.Targets) != tc.targets {
				t.Fatalf("uploads=%+v", cfg.Uploads)
			}
			if tc.targets == 1 {
				...
				if cfg.Uploads.Targets[0].Path != want {
					t.Fatalf("target=%q want %q", cfg.Uploads.Targets[0].Path, want)
				}
				...
				if !info.IsDir() {
					t.Fatalf("upload target is not a directory: %s", want)
				}
				if runtime.GOOS != "windows" && info.Mode().Perm() != 0700 {
					t.Fatalf("upload permissions: %v", info.Mode())
				}
			} else if _, err := os.Stat(filepath.Join(root, "uploads")); !os.IsNotExist(err) {
				t.Fatalf("unexpected default upload directory: %v", err)
			}
		})
	}
}
...
func TestLoadConfigExplicitCustomUploadTarget(t *testing.T) {
	...
	if len(cfg.Uploads.Targets) != 1 || cfg.Uploads.Targets[0].Path != custom {
		t.Fatalf("custom target overridden: %+v", cfg.Uploads.Targets)
	}
	if _, err := os.Stat(filepath.Join(root, "uploads")); !os.IsNotExist(err) {
		t.Fatalf("implicit target unexpectedly provisioned: %v", err)
	}
}
...
func TestGeneratedConfigKeepsAuthAwareUploadsImplicit(t *testing.T) {
	...
	if len(root.Content) != 1 || root.Content[0].Kind != yaml.MappingNode {
		t.Fatal("generated YAML root is not mapping")
	}
	for i := 0; i+1 < len(root.Content[0].Content); i += 2 {
		if root.Content[0].Content[i].Value != "uploads" {
			continue
		}
		fields := root.Content[0].Content[i+1]
		for j := 0; j+1 < len(fields.Content); j += 2 {
			if fields.Content[j].Value == "enabled" || fields.Content[j].Value == "targets" {
				t.Fatalf("generated YAML freezes implicit %q", fields.Content[j].Value)
			}
		}
		return
	}
