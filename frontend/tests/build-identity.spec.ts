...
test('development login identity links docs to the exact revision', async ({ page }) => {
  ...
  const setupLink = page.locator('.login-v2-first-run').getByRole('link', { name: 'docs' });
  await expect(setupLink).toHaveAttribute('href', `https://github.com/fiso64/gooru/blob/${revision}/docs/SERVE.md`);
  await expect(setupLink).toHaveCSS('text-decoration-line', 'underline');
