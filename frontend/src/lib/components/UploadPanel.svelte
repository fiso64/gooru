          ...
          <div class="field-control upload-initial-tags-field">
            <div class="upload-initial-tags-row">
              ...
              <div class="upload-initial-tag-actions" aria-label="Edit tags on staged files">
                <button class="g-btn g-btn-sm upload-initial-tag-symbol" type="button" disabled={stagedRows.length === 0} aria-label="Add tags to staged files" title="Add tags to staged files" onclick={() => openStagedTagDialog('add')}>
                  <Icon name="plus" size={14} />
                </button>
                <button class="g-btn g-btn-sm upload-initial-tag-symbol" type="button" disabled={stagedRows.length === 0} aria-label="Remove tags from staged files" title="Remove tags from staged files" onclick={() => openStagedTagDialog('remove')}>
