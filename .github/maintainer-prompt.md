...
- **`develop` (GitHub's default branch):** Active engineering integration and PR target. Base task branches on the appropriate current revision, normally `develop`; merge validated, permitted work into `develop`. It may contain features, fixes, documentation, and packaging that have not been released. GitHub's default-branch setting does **not** make `develop` a stable installation channel. Respect owner holds even when a PR targets `develop`.
- **`main`:** Stable **released** source branch, not a rolling mirror of `develop` and not an intermediate validation checkpoint. Advance `main` **only as part of publishing an official release**, when a coherent candidate from `develop` has passed the entire release gate (including the intended installable artifacts, their documentation, and required Nix/Cachix publication). A green CI run, merged PR, docs-only change, or successful native Cachix build on `develop` is **not** permission to advance `main`. Avoid updating `main` ahead of downloadable release artifacts. Prepare a reviewed release candidate, integrate it into `main` without force or loss of unrelated work when possible, then publish and verify the matching immutable version tag, GitHub Release artifacts, and architecture-specific Cachix outputs. If publication fails, treat `main` as an incomplete release state and resolve it before any further promotion; do not describe it as a completed release.
- **NixOS installation:** Users track `github:fiso64/gooru/main` with `flake.lock` and update using `nix flake update gooru`. This must resolve to the most recently published release, **not** unshipped engineering changes. Publishing both native Cachix packages is part of the release gate, not a reason to synchronize `develop` to `main`.
- **Immutable `vMAJOR.MINOR.PATCH` tags:** Mark officially released commits on `main`; do not move an existing published tag. The tagged release workflow publishes matching versioned artifacts and Nix packages. The legacy `release` branch is neither an active publishing nor an installation channel.

**Existing divergence needs explicit reconciliation:** Earlier maintenance mistakenly advanced `main` beyond the published v0.1.0 tag, including an unreleased Debian service change. Do not force-reset `main`, rewrite the existing tag, or assume the mismatch is acceptable. Reconcile the current `main` revision through an appropriately verified subsequent release, with correct version/changelog and downloaded artifacts, before advancing it again. Until then be explicit that the currently published `.deb` does not match the source on `main`.
