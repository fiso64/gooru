...
- **Actions runners:** this repository's canonical CI intentionally targets `[self-hosted, gooru]`. A live diagnostic on 2026-09-08 confirmed runner `server-gooru-3` (Actions runner 2.336.0, Linux x64) accepts and completes jobs normally. Prefer `[self-hosted, gooru]` for temporary maintainer validation/execution too. Do not use `ubuntu-latest` as a fallback merely for convenience: repeated temporary `ubuntu-latest` runs failed before job creation (empty jobs list), which is upstream of runner scheduling and is not evidence that the self-hosted runner is broken. If an Actions run fails with zero jobs, inspect the workflow/run metadata and runner target before retrying; do not repeatedly retry the same runner path without new evidence.
