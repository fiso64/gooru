...
root="$stage/gooru"
mkdir -p "$root/DEBIAN" "$root/usr/bin" "$root/usr/sbin" "$root/usr/share/gooru/frontend" "$root/usr/share/doc/gooru" "$root/lib/systemd/system" "$stage/work/debian"
...
install -m 0755 scripts/gooru-instance.sh "$root/usr/sbin/gooru-instance"
